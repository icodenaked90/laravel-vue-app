...
export default {
  ...
  methods: {
      ...
      buyVehicle(vehicle) {
          ...
          const userCash = Math.round(parseFloat(this.userCash) * 100) / 100; // Round to 2 decimal places
          const vehicleCost =
              Math.round(parseFloat(vehicle.vehiclecost) * 100) / 100; // Round to 2 decimal places

          if (
              !isNaN(userCash) &&
              !isNaN(vehicleCost) &&
              userCash >= vehicleCost
          ) {
              axios
